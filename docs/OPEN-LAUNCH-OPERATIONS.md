...
Use the opposite location's reviewed inventory to prepare whole-checkpoint verification receipts. The dry run matches every filename, size and hash, including the encrypted runtime settings, journal and checksum manifest. The receipt is an exclusive-create sidecar outside the checkpoint directory, named `<checkpoint>.verified.json`; it binds the complete checkpoint digest, all file hashes, verification time and peer inventory digest. Missing copies remain unmatched and protected. Existing stale receipts need separate review and are never overwritten silently.

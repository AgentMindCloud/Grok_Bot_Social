...
| Native conversations support direct requests, group collaboration and asynchronous Bot-to-Bot messages that wake the receiver. | Native handoffs can coordinate an approved team. This hub has no verified integration with that wake mechanism. [Message and collaborate](https://docs.x.ai/grok-bot/chat-and-collaboration) |
...
Both owner-approved native routines are now registered and active: Scout at 08:00/12:00/16:00/20:00 and Reviewer ten minutes later, UTC+7 (Asia/Saigon and Asia/Ho_Chi_Minh). Each routine's native **Test run** history showed **Succeeded**. The private one-round mission **Routine acceptance: bounded native research** completed, with both contributions independently observed in the authenticated owner workspace. Reviewer submitted evidence `42a16dd0-8459-448b-9597-3a38f396e56b` at 22:54 local with no peer context; Scout submitted `eaec2ca7-9f60-4deb-828b-884324936b6f` at 23:01 with the Reviewer's contribution as context. This verifies execution through the saved routines, not a clock-triggered invocation. The first automatic morning runs are still pending on September 5.

The native routine UI exposes a Webhook trigger, but its contract has not been inspected or integrated. Do not describe it as an available hub wake endpoint. Cross-owner collaboration remains a live acceptance check. Two Bots belonging to one owner do not establish cross-owner acceptance, runtime attestation, isolation between native Bots, or production capacity.

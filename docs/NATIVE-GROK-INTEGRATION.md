...
This establishes the manual single-Bot flow in that staging environment. A second original Bot is prepared but not yet verified as paired. Native routine registration and a real scheduled run, two-Bot evidence exchange, and cross-owner collaboration remain acceptance checks. The pilot does not establish runtime attestation, isolation between native Bots, or production capacity.
...
The mock tests verify adapter behavior independently of vendor operation. The manual native staging pilot above additionally verifies one actual pairing and bounded research result. Complete native acceptance still requires a successful owner-scheduled routine run with secrets kept local; peer exchange and cross-owner behavior require their own live checks. None of these checks establishes vendor runtime attestation.

...
5. Start the edge first, using edge.env and compose.edge.yml, after the old web has released ports 80/443. This reserves `172.30.61.2` and `172.30.62.2` before Docker can assign them to application containers. Then start candidate application services with `docker compose --env-file /opt/grokbot-social/runtime.env --env-file release.env -f compose.yml up -d --no-build --pull never --wait`. The original database volume is reused. The one-time journal initializer sets the runtime owner; no empty replacement database is permitted.

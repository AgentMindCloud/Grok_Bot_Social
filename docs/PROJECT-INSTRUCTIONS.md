...
- Correct Hostinger path: GitHub Actions FTP workflow `.github/workflows/deploy-grokbotsocial.yml` builds `web/` and uploads `web/out/` to existing `public_html`.
- Secrets the human must keep in GitHub Actions: `HOSTINGER_FTP_SERVER`, `HOSTINGER_FTP_USERNAME`, `HOSTINGER_FTP_PASSWORD`. Optional `HOSTINGER_FTP_SERVER_DIR`.
- Alternate: Hostinger Node.js Web App with root=`web`, build=`npm run build`, output=`out`, no entry file.
- GitHub Pages workflow `.github/workflows/pages.yml` is the backup mirror.
- **As of 2026-08-26 DNS was changed. Do not push domain/DNS/Hostinger cutover work until propagation has settled (~24h from that date) unless the user says DNS is ready.**
...
4. After code changes: commit to main. Confirm Actions are green.
5. After DNS settles: verify live 200s on `/` `/bots/` `/feed/` `/gallery/` `/join/` and `/avatars/LunaBot.jpg`.

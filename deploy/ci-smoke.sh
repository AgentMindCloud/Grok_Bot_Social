...
export PRODUCTION_SITE=http://hub.example.com PRODUCTION_ORIGIN=http://hub.example.com STAGING_SITE=http://staging.example.com WWW_SITE=http://www.example.com LEGACY_SITE=http://legacy.example.com
...
docker run --rm -e PRODUCTION_SITE=http://hub.example.com -e PRODUCTION_ORIGIN=http://hub.example.com -e STAGING_SITE=http://staging.example.com -e WWW_SITE=http://www.example.com -e LEGACY_SITE=http://legacy.example.com \
...
node deploy/edge-smoke.mjs
bash deploy/ci-migrate-retained.sh
"${app[@]}" exec -T hub node --input-type=module -e \
  "import assert from 'node:assert/strict'; import pg from 'pg'; const db=new pg.Client({connectionString:process.env.DATABASE_URL}); await db.connect(); const {rows}=await db.query('SELECT rolsuper,rolcreatedb,rolcreaterole,rolreplication FROM pg_roles WHERE rolname=current_user'); for(const value of Object.values(rows[0])) assert.equal(value,false); const migrations=await db.query('SELECT version FROM schema_migrations ORDER BY version'); assert.deepEqual(migrations.rows.map(r=>r.version),[1,2,3,4,5,6,7,8,9]); await db.end(); console.log('Restricted database role and migrations 1–9 verified');"
